...
class Image {
	...
	public function __construct($image, $config = NULL)
	{
		...
		if ($check === FALSE)
			throw new RxException('image getimagesize missing');

		// Check to make sure the image exists
		if ( ! is_file($image))
			throw new RxException('image file not found');
		...
		if ( ! is_array($image_info) OR count($image_info) < 3)
			throw new RxException('image file unreadable: '. str_replace('\\', '/', realpath($image)));

		// Check to make sure the image type is allowed
		if ( ! isset(Image::$allowed_types[$image_info[2]])) {
			throw new RxException('image type not allowed: '. str_replace('\\', '/', realpath($image)));
        }
		...
		if ( ! ($this->driver instanceof Image_Driver))
			throw new RxException('image driver must be implement Image_Driver class');
	}
	...
	public function __get($property)
	{
		...
		{
			throw new RxException('invalid property');
		}
	}
	...
	public function resize($width, $height, $master = NULL)
	{
		if ( ! $this->valid_size('width', $width))
			throw new RxException('image invalid width');

		if ( ! $this->valid_size('height', $height))
			throw new RxException('image invalid height');

		if (empty($width) AND empty($height))
			throw new RxException('image invalid dimensions');
		...
		elseif ( ! $this->valid_size('master', $master))
			throw new RxException('image invalid master');
	...
	{
		if ( ! $this->valid_size('width', $width))
			throw new RxException('image invalid width', $width);

		if ( ! $this->valid_size('height', $height))
			throw new RxException('image invalid height', $height);

		if ( ! $this->valid_size('top', $top))
			throw new RxException('image invalid top', $top);

		if ( ! $this->valid_size('left', $left))
			throw new RxException('image invalid left', $left);
	...
	public function flip($direction)
	{
		if ($direction !== self::HORIZONTAL AND $direction !== self::VERTICAL)
			throw new RxException('image invalid flip');
	...
	public function save($new_image = FALSE, $chmod = 0644, $keep_actions = FALSE)
	{
		...
		if ( ! is_writable($dir))
			throw new RxException('image directory unwritable');
