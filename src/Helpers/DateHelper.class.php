...
class DateHelper
{
	...
	*/
	...
	public static function niceDate($dateString = null)
	{
		...
		return date("j", $date).' '.$aMonth[$nMonth].' '.date("Y", $date);
	}

	public static function niceDateTime($dateString = null)
	{
		$date = ($dateString == null) ? time() : strtotime($dateString);
		...
		return date("d", $date).' '.$aMonth[$nMonth].' '.date("Y", $date).' '.date("H:i", $date);
	}
